...
const VideoDetail = () => {
  ...
  useEffect(() => {
    const fetchVideo = async () => {
      try {
        console.log('🎬 [VIDEO DETAIL DEBUG] ========================================');
        console.log('🎬 [VIDEO DETAIL DEBUG] Starting fetchVideo for ID:', id);
        setLoading(true);
        setError(null);
        
        const videoData = await apiService.getVideoById(id);
        console.log('🎬 [VIDEO DETAIL DEBUG] API response:', videoData);
        console.log('🎬 [VIDEO DETAIL DEBUG] Video ID in response:', videoData?.id);
        console.log('🎬 [VIDEO DETAIL DEBUG] Video title in response:', videoData?.title);
        
        setVideo(videoData);
        console.log('🎬 [VIDEO DETAIL DEBUG] Video state set to:', videoData);
      } catch (err) {
        console.error('❌ [VIDEO DETAIL DEBUG] Error fetching video:', err);
        console.error('❌ [VIDEO DETAIL DEBUG] Error details:', err.response?.data);
        setError(err);
      } finally {
        setLoading(false);
      }
    ...
    console.log('🎬 [VIDEO DETAIL DEBUG] useEffect triggered');
    console.log('🎬 [VIDEO DETAIL DEBUG] URL param ID:', id);
    if (id) {
      fetchVideo();
    } else {
      console.log('❌ [VIDEO DETAIL DEBUG] No ID found in URL params');
    }
  }, [id]);
  ...
  const handlePlay = () => {
    console.log('🎬 [VIDEO DETAIL DEBUG] Play button clicked for video:', video);
    console.log('🎬 [VIDEO DETAIL DEBUG] Video ID:', video?.id);
    console.log('🎬 [VIDEO DETAIL DEBUG] Video title:', video?.title);
    setShowPlayer(true);
  };
